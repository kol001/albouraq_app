...
// import type { Privilege as PrivilegeType } from '../../app/privilegesSlice';
...
const AutorisationPage = () => {
  ...
  const [activeModal, setActiveModal] = useState<'none' | 'create' | 'assign-profile' | 'assign-module'>('none');
  ...
  const handleSubmitCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    const result = await dispatch(createAutorisation({ nom, profileId: selectedProfileId, privilegeId: selectedPrivilegeId }));
    if (createAutorisation.fulfilled.match(result)) {
      handleSuccess('Autorisation créée !');
      setNom(''); setSelectedProfileId(''); setSelectedPrivilegeId('');
  ...
  const handleSubmitAssignProfile = async (e: React.FormEvent) => {
    ...
    if (assignProfileToAutorisation.fulfilled.match(result)) {
      ...
      setSelectedAuthId(''); setAssignProfileId('');
  ...
  const handleSubmitAssignModule = async (e: React.FormEvent) => {
    e.preventDefault();
    const result = await dispatch(assignModuleToAutorisation({ authId: selectedAuthIdModule, moduleId: selectedModuleId }));
    if (assignModuleToAutorisation.fulfilled.match(result)) {
      handleSuccess('Module assigné !');
      setSelectedAuthIdModule(''); setSelectedModuleId('');
      dispatch(fetchAutorisations());
    }
  };
  ...
  return (
    <div className="p-8 max-w-[1600px] mx-auto">
      {/* HEADER AVEC ACTIONS DYNAMIQUES */}
            ...
            {autorisations.map((aut: Autorisation) => (
              <tr key={aut.id} className="hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-6 py-4 text-right space-x-2">
                  <button onClick={() => aut.status === 'ACTIF' ? dispatch(deactivateAutorisation({authId: aut.id})) : dispatch(activateAutorisation({authId: aut.id}))} className={`text-xs font-bold ${aut.status === 'ACTIF' ? 'text-amber-600' : 'text-green-600'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center  backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden scale-in-center">
            <div className={`p-6 border-b flex justify-between items-center ${
              activeModal === 'create' ? 'bg-indigo-50' : activeModal === 'assign-profile' ? 'bg-blue-50' : 'bg-purple-50'
            }`}>
            ...
            <div className="p-8">
              ...
              {activeModal === 'create' && (
                <form onSubmit={handleSubmitCreate} className="space-y-4">
                  ...
                  <select value={selectedProfileId} onChange={(e) => setSelectedProfileId(e.target.value)} className="w-full p-3 border rounded-xl bg-white focus:ring-2 focus:ring-indigo-500" required>
              ...
              {activeModal === 'assign-module' && (
                <form onSubmit={handleSubmitAssignModule} className="space-y-4">
                  ...
                  <select value={selectedModuleId} onChange={(e) => setSelectedModuleId(e.target.value)} className="w-full p-3 border rounded-xl bg-white focus:ring-2 focus:ring-purple-500" required>
